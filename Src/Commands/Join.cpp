...
void Execute::join(int &fd, Server *server, strvector splitted)
{
    ...
    if(server->getServerChannel(splitted[1])->getUser(user->getNickName()) == NULL)
    {
		...
		std::cout << "Channeldaki Userlar\n";

		for (size_t i = 0; i < server->getServerChannel(splitted[1])->getUsers().size(); i++)
			std::cout << server->getServerChannel(splitted[1])->getUsers()[i]->getNickName() << std::endl;

        user->addChannel(server->getServerChannel(splitted[1]));
		std::cout << user->getNickName() << " userinin ait oldugu channellar: ";

		for (size_t i = 0; i < user->getChannels().size(); i++)
			std::cout << user->getChannels()[i]->getName() << " ";
		std::cout << std::endl;
